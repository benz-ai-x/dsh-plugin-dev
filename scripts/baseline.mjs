...
const TOOL_LINKED_PACKAGES = Object.freeze([
  '@deepseek-ai/cordis',
...
const TOOL_PUBLICATION_ROOTS = Object.freeze([
...
const BASELINE_CHANNELS = Object.freeze(['stable', 'edge'])
...
function assertChannel(channel) {
  if (!BASELINE_CHANNELS.includes(channel)) {
  ...
  for (const packageName of closure) {
    ...
    for (const section of DEPENDENCY_SECTIONS) {
      for (const [name, range] of Object.entries(entry.manifest[section] ?? {})) {
        ...
        if (!requirements.has(name)) requirements.set(name, new Set())
        requirements.get(name).add(range)
      }
    }
  }
...
export function loadLockedChannel(root = projectRoot, requestedChannel) {
  ...
  if (lock.schemaVersion === 2) {
    catalogPath = resolveProjectArtifact(baseline.catalog?.path, `${channel} catalog`)
    ...
    if (digest !== baseline.catalog.sha256) {
      fail(
        'DSH_BASELINE_CATALOG_MISMATCH',
        `${channel} catalog digest mismatch: expected ${baseline.catalog.sha256}, received ${digest}`,
      )
    }
...
export function updateChannel({ harnessRoot, channel = 'edge', tag, verifiedOn = new Date().toISOString().slice(0, 10) }) {
  assertChannel(channel)
  ...
  if (existing.schemaVersion === 1) {
    const channels = {}
    for (const initialChannel of ['stable', 'edge']) {
...
export function diffCatalogs(fromCatalog, toCatalog) {
  ...
  const changedSkills = [...toSkills.keys()].filter(path => {
    ...
    return previous && previous.digest !== toSkills.get(path).digest
  }).sort()
  const upstream = {}
  for (const field of ['tag', 'version', 'node', 'packageManager', 'commit', 'docsDigest']) {
...
function npmView(requirement, registry) {
  return new Promise(resolveResult => {
    ...
    const child = spawn('npm', args, { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] })
    let stdout = ''
    let stderr = ''
...
export function checkLockedChannel(channel, explicitRoot) {
  ...
  if (actual !== loaded.baseline.catalog.sha256) {
    fail(
      'DSH_BASELINE_CATALOG_DRIFT',
      `${loaded.channel} source scans to ${actual}, expected ${loaded.baseline.catalog.sha256}; run upstream:diff before updating edge`,
    )
  }
  verifySnapshotSkills(loaded)
  return { loaded, harnessRoot, scanned }
}
...
export function promoteChannel(from = 'edge', to = 'stable') {
  ...
  if (from === to) fail('DSH_BASELINE_PROMOTION_INVALID', 'promotion channels must differ')
  const source = preflightChannel(from)
  const registry = { ...validateReport(source, 'registry', 'ready'), channel: to }
  const verification = { ...validateReport(source, 'verification', 'passed'), channel: to }
  copyChannelFiles(source, to, registry, verification)
  verification.projectDigest = projectContractDigest(projectRoot)
  writeJsonAtomic(join(projectRoot, artifactPaths(to).verification), verification)
  const updated = structuredClone(source.lock)
  updated.channels[to] = channelRecord(
    source.catalog,
    to,
...
function diffText(diff, from, to) {
  ...
  for (const section of ['packages', 'skills', 'toolClosure']) {
    for (const kind of ['added', 'removed', 'changed']) {
      const values = diff[section]?.[kind]
