...
const BASELINE_CHANNELS = ['stable', 'edge'];
...
function resolveProjectArtifact(path, label) {
    if (typeof path !== 'string' || !path)
        fail('DSH_BASELINE_INVALID', `${label} path is missing`);
...
function readJson(path, code = 'DSH_BASELINE_INVALID') {
    try {
...
function runGit(root, args, code = 'DSH_BASELINE_GIT') {
...
export function harnessWorktreeChanges(root) {
    const commands = [
...
export function projectContractDigest(root = projectRoot) {
    const excluded = (path) => (path === 'dsh-reference.lock.json'
    ...
    for (const path of files)
        aggregate.update(`${sha256(readFileSync(join(root, path)))}  ${path}\n`);
    return aggregate.digest('hex');
}
function workspaceManifestPaths(root) {
    const paths = new Set();
    const addManifest = (directory) => {
        const path = join(directory, 'package.json');
        if (existsSync(path))
            paths.add(path);
    };
    for (const group of listDirectories(join(root, 'packages'))) {
        for (const packageRoot of listDirectories(group))
            addManifest(packageRoot);
    }
    for (const packageRoot of listDirectories(join(root, 'vendor')))
        addManifest(packageRoot);
    for (const appRoot of listDirectories(join(root, 'apps')))
        addManifest(appRoot);
...
function releaseFamily(path) {
    if (path.startsWith('vendor/'))
        return 'vendor';
    if (path.startsWith('apps/'))
        return 'dsh';
    if (path.startsWith('packages/experimental/'))
        return 'experimental';
...
function selectedManifest(manifest) {
    ...
    for (const section of DEPENDENCY_SECTIONS)
        selected[section] = dependencyMap(manifest[section]);
    return sortObject(selected);
}
function packageEntries(root) {
    const names = new Map();
    const entries = [];
    for (const manifestPath of workspaceManifestPaths(root)) {
        const manifest = readJson(manifestPath, 'DSH_BASELINE_SOURCE_INVALID');
        ...
        if (names.has(manifest.name)) {
            fail('DSH_BASELINE_SOURCE_INVALID', `duplicate workspace package ${manifest.name}: ${names.get(manifest.name)} and ${path}`);
        }
...
function skillRole(path) {
    if (path.startsWith(PRODUCT_SKILL_PREFIX) && path.includes('/skills/'))
        return 'product';
...
export function computeCapabilityClosure(packages, roots) {
    ...
    const visit = (name) => {
        ...
        if (entry === undefined)
            fail('DSH_BASELINE_SOURCE_INVALID', `capability package is missing: ${name}`);
        for (const section of DEPENDENCY_SECTIONS) {
...
function externalRequirements(packages, closure) {
    const byName = new Map(packages.map(entry => [entry.name, entry]));
    const requirements = new Map();
    for (const packageName of closure) {
        const entry = byName.get(packageName);
        if (entry === undefined)
            fail('DSH_BASELINE_SOURCE_INVALID', `closure package is missing: ${packageName}`);
        for (const section of DEPENDENCY_SECTIONS) {
...
export function scanHarness(root, options = {}) {
    ...
    const rootManifest = readJson(join(harnessRoot, 'package.json'), 'DSH_BASELINE_SOURCE_INVALID');
    ...
    const tagCommit = runGit(harnessRoot, ['rev-parse', `${tag}^{commit}`], 'DSH_BASELINE_TAG_MISMATCH');
    if (tagCommit !== commit) {
    ...
    const linkedPackages = Object.fromEntries(TOOL_LINKED_PACKAGES.map(name => {
        ...
        if (!entry)
            fail('DSH_BASELINE_SOURCE_INVALID', `linked Tool package is missing: ${name}`);
...
function compactBaselineFromV1(lock) {
...
export function selectChannel(lock, requestedChannel) {
    ...
    if (!baseline) {
        fail('DSH_BASELINE_CHANNEL_UNKNOWN', `unknown baseline channel ${channel}; available: ${Object.keys(lock.channels).sort().join(', ')}`);
    }
    return { channel, baseline };
}
export function loadLockedChannel(root = projectRoot, requestedChannel) {
    ...
    if (lock.schemaVersion === 2) {
        ...
        if (!existsSync(catalogPath))
            fail('DSH_BASELINE_CATALOG_MISSING', `missing catalog for ${channel}: ${catalogPath}`);
        catalogText = readFileSync(catalogPath, 'utf8');
        const digest = sha256(catalogText);
        if (digest !== catalogSummary.sha256) {
            fail('DSH_BASELINE_CATALOG_MISMATCH', `${channel} catalog digest mismatch: expected ${catalogSummary.sha256}, received ${digest}`);
        }
        catalog = JSON.parse(catalogText);
        for (const field of ['tag', 'version', 'node', 'packageManager', 'commit', 'docsDigest']) {
...
function reportSummary(path, report) {
...
function uncheckedRegistry(channel, catalogDigest, capability = 'tool') {
    return {
        schemaVersion: 1,
        channel,
        capability,
...
function unverifiedBaseline(channel, catalogDigest) {
    return {
        schemaVersion: 1,
        channel,
        catalogSha256: catalogDigest,
...
function artifactPaths(channel) {
...
function copyProductSkills(harnessRoot, catalog, channel) {
...
function channelRecord(catalog, channel, harnessRoot, verifiedOn, registry, verification) {
    const paths = artifactPaths(channel);
...
function writeChannelArtifacts(harnessRoot, catalog, channel, registry, verification) {
    const paths = artifactPaths(channel);
...
export function updateChannel({ harnessRoot, channel: requestedChannel = 'edge', tag, verifiedOn = new Date().toISOString().slice(0, 10), }) {
    const channel = assertChannel(requestedChannel);
    const root = resolve(harnessRoot);
    const existing = readJson(defaultLockPath);
    ...
    if (existing.schemaVersion === 1) {
        ...
        for (const initialChannel of BASELINE_CHANNELS) {
            ...
            channels[initialChannel] = channelRecord(catalog, initialChannel, root, verifiedOn, registry, verification);
        }
    ...
    if (updated.channels === undefined)
        fail('DSH_BASELINE_LOCK_UNSUPPORTED', 'schema v2 channels are missing');
...
export function diffCatalogs(fromCatalog, toCatalog) {
    ...
    const changedPackages = [...toPackages.keys()].filter(name => {
        ...
        return previous && (previous.version !== toPackages.get(name).version
...
function npmView(requirement, registry) {
...
export async function buildRegistryReport({ channel, catalog, catalogSha256, capability = 'tool', registry, resolver = npmView, checkedAt = new Date().toISOString(), }) {
    const contract = catalog.capabilities?.[capability];
    if (!contract)
        fail('DSH_BASELINE_CAPABILITY_UNKNOWN', `catalog has no ${capability} capability`);
    const byName = new Map(catalog.packages.map(entry => [entry.name, entry]));
    const internal = contract.publicationClosure.map(name => {
        const entry = byName.get(name);
        if (entry === undefined)
            fail('DSH_BASELINE_SOURCE_INVALID', `publication package is missing: ${name}`);
        return {
            kind: 'upstream',
            name,
    ...
    const external = contract.externalRequirements.flatMap(entry => entry.ranges.map(range => ({
        kind: 'external',
        name: entry.name,
        requirement: `${entry.name}@${range}`,
        range,
    })));
    const requirements = [...internal, ...external];
    const checked = await mapConcurrent(requirements, 6, async (requirement) => {
        if (requirement.private)
            return { ...requirement, available: false, reason: 'package is private' };
...
function updateReportInLock(loaded, kind, report) {
...
function verifySnapshotSkills(loaded) {
    ...
    for (const skill of productSkills) {
        ...
        for (const file of skill.files) {
            ...
            if (!existsSync(snapshotPath) || sha256(readFileSync(snapshotPath)) !== file.sha256) {
                fail('DSH_BASELINE_SKILL_MISMATCH', `${loaded.channel} product Skill snapshot differs: ${skill.name}/${file.path}`);
            }
        }
    }
}
export function resolveHarnessRoot(loaded, explicitRoot) {
...
export function checkLockedChannel(channel, explicitRoot) {
    const loaded = loadLockedChannel(projectRoot, channel);
    if (!loaded.catalog)
        fail('DSH_BASELINE_LOCK_UNSUPPORTED', 'upgrade schema v1 before checking a channel');
    const catalogSummary = loaded.baseline.catalog;
    if (catalogSummary === undefined)
        fail('DSH_BASELINE_CATALOG_MISSING', `${loaded.channel} has no catalog summary`);
    const typedLoaded = loaded;
    ...
    if (actual !== catalogSummary.sha256) {
        fail('DSH_BASELINE_CATALOG_DRIFT', `${loaded.channel} source scans to ${actual}, expected ${catalogSummary.sha256}; run upstream:diff before updating edge`);
    }
    verifySnapshotSkills(typedLoaded);
    return { loaded: typedLoaded, harnessRoot, scanned };
}
function validateReport(loaded, kind, requiredStatus) {
    ...
    if (!existsSync(path))
        fail('DSH_BASELINE_REPORT_MISSING', `missing ${loaded.channel} ${kind} report`);
    ...
    if (requiredStatus && report.status !== requiredStatus) {
        fail('DSH_BASELINE_PREFLIGHT_BLOCKED', `${loaded.channel} ${kind} status is ${report.status}; required ${requiredStatus}`);
    }
    if (kind === 'verification' && report.status === 'passed') {
        ...
        if (report.projectDigest !== currentDigest) {
            fail('DSH_BASELINE_REPORT_STALE', `${loaded.channel} verification targets project ${report.projectDigest}, current contract is ${currentDigest}`);
        }
    }
    return report;
}
...
export function promoteChannel(from = 'edge', to = 'stable') {
    ...
    if (source.catalog === undefined)
        fail('DSH_BASELINE_CATALOG_MISSING', `${fromChannel} has no catalog`);
    ...
    if (updated.channels === undefined)
        fail('DSH_BASELINE_LOCK_UNSUPPORTED', 'promotion requires schema v2');
    updated.channels[toChannel] = channelRecord(typedSource.catalog, toChannel, resolveHarnessRoot(source), new Date().toISOString().slice(0, 10), registry, verification);
    writeJsonAtomic(defaultLockPath, updated);
    return updated;
}
...
export function verifyChannel(channel = 'edge', explicitRoot) {
...
function parseArguments(argv) {
    ...
    for (let index = 0; index < rest.length; index += 1) {
        ...
        if (!key)
            fail('DSH_BASELINE_USAGE', `unknown argument: ${argument}`);
        const value = rest[index + 1];
        if (!value)
            fail('DSH_BASELINE_USAGE', `${argument} requires a value`);
        options[key] = value;
        index += 1;
    }
    return options;
}
function diffText(diff, from, to) {
    ...
    for (const section of ['packages', 'skills', 'toolClosure']) {
        for (const kind of ['added', 'removed', 'changed']) {
            const values = kind === 'changed' && section === 'toolClosure'
                ? undefined
                : diff[section][kind];
            if (values?.length)
                lines.push(`${section}.${kind} (${values.length}): ${values.join(', ')}`);
        }
    }
    if (lines.length === 1)
...
async function main() {
    ...
    if (options.command === 'scan') {
        ...
        if (!root)
            fail('DSH_BASELINE_USAGE', 'scan requires --harness-root or DSH_HARNESS_BASELINE_ROOT');
    ...
    if (options.command === 'update') {
        const root = options.harnessRoot || process.env.DSH_HARNESS_BASELINE_ROOT || process.env.DSH_HARNESS_ROOT;
        if (!root)
            fail('DSH_BASELINE_USAGE', 'update requires --harness-root or DSH_HARNESS_BASELINE_ROOT');
