...
async function planUserSkillInstall({ source, targetRoot, }) {
    ...
    if (!sourceState?.isDirectory()) {
        throw new SkillInstallError('DSH_SKILL_SOURCE_MISSING', `canonical skill directory not found: ${resolvedSource}`);
    }
    ...
    if (targetState !== undefined) {
        ...
        throw new SkillInstallError('DSH_SKILL_INSTALL_CONFLICT', `refusing to replace ${target}; it is ${detail}`);
    }
    return { status: 'would-install', source: resolvedSource, target, targetRoot: resolvedTargetRoot };
}
async function materializePlan(plan, dryRun) {
    if (plan.status === 'already-installed' || dryRun)
        return plan;
...
export async function installUserSkill({ source = defaultSource, targetRoot = join(homedir(), '.agents', 'skills'), dryRun = false, } = {}) {
    const plan = await planUserSkillInstall({ source, targetRoot });
    return materializePlan(plan, dryRun);
}
export async function installUserSkills({ source = defaultSource, homeDirectory = homedir(), agents = ['codex', 'claude'], dryRun = false, } = {}) {
    ...
    for (const agent of selectedAgents) {
        if (!(agent in agentTargetSegments)) {
            throw new SkillInstallError('DSH_SKILL_INSTALL_USAGE', `unsupported target agent: ${agent}; expected codex or claude`);
        }
    }
    ...
    for (const plan of plans)
        results.push(await materializePlan(plan, dryRun));
    return results;
}
function usage() {
