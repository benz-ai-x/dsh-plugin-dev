...
function nodeSatisfies(range, version = process.version) {
    ...
    return range.split('||').some(rawClause => {
        ...
        if (clause.startsWith('>='))
            return true;
        if (clause.startsWith('^'))
            return compareVersions(actual, [minimum[0] + 1, 0, 0]) < 0;
        return compareVersions(actual, minimum) === 0;
    });
}
const defaultLinkedPackageLocations = {
...
function validateLinkedArtifacts(sourceRoot, linkedPackageLocations) {
    ...
    for (const [packageName, sourcePath] of Object.entries(locations)) {
        ...
        if (existsSync(sourceDirectory))
            inputs.push(...listFiles(sourceDirectory));
...
const expectedToolingArtifacts = new Map([
...
const toolingArtifacts = parseJson('tooling-artifacts.json');
if (toolingArtifacts !== undefined) {
    ...
    check(toolingArtifacts.artifacts.length === expectedToolingArtifacts.size, 'tooling artifact manifest covers every TypeScript entry');
...
for (const markdownFile of markdownFiles) {
    ...
    for (const match of content.matchAll(/\[[^\]]*\]\(([^)]+)\)/g)) {
        ...
        check(existsSync(resolved), `${relative(projectRoot, markdownFile).split(sep).join('/')} link ${target} resolves`);
    }
}
...
for (const [label, frontmatter] of [
    ...
    check(frontmatter.get('description') === canonicalFrontmatter.get('description'), `${label} skill description matches the canonical skill`);
}
for (const [path, content] of [
    ...
    check(existsSync(resolve(dirname(projectPath(path)), target)), `${path} target resolves to the canonical skill`);
...
check(canonical.includes('scripts/create-project.mjs'), 'canonical skill routes new Tool projects through the deterministic generator');
...
if (manifest) {
    ...
    check(manifest.scripts?.['context:check:strict'] === 'node scripts/verify-context.mjs --require-source', 'strict context script requires the source baseline');
...
if (lock && loadedBaseline) {
    ...
    const releasePackageCount = catalog?.packages?.filter(entry => ['dsh', 'vendor'].includes(entry.releaseFamily) && entry.private !== true).length;
    check(catalog?.summary?.releasePackageCount === releasePackageCount, 'catalog covers the complete public DSH and vendor release families');
    ...
    if (!existsSync(sourceRoot)) {
        ...
        if (requireSource)
            ...
            warn(`${message}; set ${environmentVariable || 'DSH_HARNESS_ROOT'} for strict validation`);
    }
    else {
        try {
