...
export class CompatibilityError extends Error {
  constructor(public code: string, message: string) { super(message); this.name = 'CompatibilityError' }
}

export function fail(code: string, message: string): never { throw new CompatibilityError(code, message) }
...
export interface Difference { field: string[]; from?: unknown; to?: unknown }
function differences(left: unknown, right: unknown, field: string[]): Difference[] {
  if (JSON.stringify(sorted(left)) === JSON.stringify(sorted(right))) return []
  if (isObject(left) && isObject(right)) return [...new Set([...Object.keys(left), ...Object.keys(right)])].sort()
    .flatMap(key => differences(left[key], right[key], [...field, key]))
  return [{ field, ...(left !== undefined ? { from: left } : {}), ...(right !== undefined ? { to: right } : {}) }]
}

export interface CompareOptions { repo: string; base: string; target?: string; roots?: readonly string[] }

export function compareRevisions(options: CompareOptions) {
  const requestedRoot = resolve(options.repo)
  const git = (args: string[], input?: string, accept = [0]) => {
  ...
  const gitText = (args: string[]) => git(args).stdout.toString('utf8')
  const resolveCommit = (ref: string) => gitText(['rev-parse', '--verify', '--end-of-options', `${ref}^{commit}`]).trim()
  ...
  const tree = (commit: string) => {
    ...
    for (const record of gitText(['ls-tree', '-r', '-z', '--full-tree', commit]).split('\0').filter(Boolean)) {
      ...
      if (type === 'blob' && oid && mode && ['100644', '100755'].includes(mode)) entries.set(record.slice(split + 1), oid)
    }
    return entries
  }
  ...
  const isManifest = (path: string) => path === 'package.json' || path.endsWith('/package.json')
  const manifestPaths = allPaths.filter(isManifest)
  const metadataPaths = [...manifestPaths, 'pnpm-workspace.yaml']
  const objectIds = [...new Set(metadataPaths.flatMap(path => [beforeTree.get(path), afterTree.get(path)]).filter((id): id is string => id !== undefined))]
  ...
  if (objectIds.length) {
    ...
    for (const oid of objectIds) {
      ...
      if (!match || match[1] !== oid) fail('COMPAT_GIT_ERROR', `Unexpected blob response for ${oid}`)
      const start = newline + 1
      const end = start + Number(match[2])
      if (end >= buffer.length || buffer[end] !== 10) fail('COMPAT_GIT_ERROR', `Incomplete blob response for ${oid}`)
      objects.set(oid, buffer.subarray(start, end).toString('utf8'))
      offset = end + 1
    }
  }
  const blob = (entries: Map<string, string>, path: string) => objects.get(entries.get(path) ?? '')
  const manifests = (entries: Map<string, string>, revision: string) => {
    const values = new Map<string, Manifest>()
    for (const path of manifestPaths.filter(path => entries.has(path))) {
      try {
        const value: unknown = JSON.parse(blob(entries, path)!.replace(/^\uFEFF/, ''))
  ...
  const packageIdentity = (path: string, value: Manifest) => ({ path, name: value.name ?? null, version: value.version ?? null, private: value.private === true })
  const added: ReturnType<typeof packageIdentity>[] = []
  const removed: ReturnType<typeof packageIdentity>[] = []
  const contractChanged: Array<{ path: string; name: unknown; fromVersion: unknown; toVersion: unknown; changes: Difference[] }> = []
  let versionOnlyCount = 0
  let unchangedManifestCount = 0
  for (const path of manifestPaths) {
    const left = before.get(path)
    const right = after.get(path)
    if ((beforeTree.has(path) && !left) || (afterTree.has(path) && !right)) continue
    if (!left && right) { added.push(packageIdentity(path, right)); continue }
    if (!right && left) { removed.push(packageIdentity(path, left)); continue }
    ...
    if (changes.length) contractChanged.push({ path, name: right.name ?? left.name ?? null, fromVersion: left.version ?? null, toVersion: right.version ?? null, changes })
    else if (left.version !== right.version) versionOnlyCount += 1
    else unchangedManifestCount += 1
  }
  const skillPaths = allPaths.filter(path => path === 'SKILL.md' || path.endsWith('/SKILL.md'))
  const skillEntrypoints = { added: [] as string[], removed: [] as string[], changed: [] as string[], unchangedCount: 0 }
  const skillResources: Array<{ entrypoint: string; changedPaths: string[] }> = []
  for (const path of skillPaths) {
    const left = beforeTree.get(path)
    const right = afterTree.get(path)
    if (!left) skillEntrypoints.added.push(path)
    else if (!right) skillEntrypoints.removed.push(path)
    else if (left !== right) skillEntrypoints.changed.push(path)
    else skillEntrypoints.unchangedCount += 1
    ...
    if (changedPaths.length) skillResources.push({ entrypoint: path, changedPaths })
  }
  ...
  for (const record of gitText(['diff', '--no-ext-diff', '--no-textconv', '--no-renames', '--numstat', '-z', base, target, '--']).split('\0').filter(Boolean)) {
    ...
    if (added === '-' || removed === '-') summary.binaryPaths += 1
    else { summary.insertions += Number(added); summary.deletions += Number(removed) }
  }
  const revision = (requestedRef: string, commit: string, manifest?: Manifest) => ({
  ...
  for (const [revision, workspace, graph] of [[base, beforeWorkspace, beforeGraph], [target, afterWorkspace, afterGraph]] as const) {
    issues.push(...[...workspace.issues, ...graph.issues].map(issue => ({ ...issue, revision })))
  }
  if (!roots.length) issues.push({ kind: 'dependency-roots', reason: 'No delivery roots selected; dependency closure is not assessed.' })
  return {
  ...
  try {
    ...
    if (args.includes('--help') || args.includes('-h')) console.log(usage)
      ...
      for (let index = 0; index < args.length; index += 1) {
        ...
        if (!['--repo', '--base', '--target', '--root-package'].includes(flag) || !value || value.startsWith('--')) fail('COMPAT_USAGE', usage)
        if (flag === '--root-package') roots.push(value)
        else values[flag.slice(2)] = value
      }
      if (!values.repo || !values.base) fail('COMPAT_USAGE', usage)
