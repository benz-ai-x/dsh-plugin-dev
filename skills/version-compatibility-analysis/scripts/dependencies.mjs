...
function pnpmPatterns(text) {
    ...
    if (head && !head.startsWith('#')) {
        try {
            const values = JSON.parse(head);
            return stringList(values) ? values : undefined;
        }
        catch {
            return undefined;
        }
    ...
    for (const line of lines.slice(start + 1)) {
        ...
        if (single)
            values.push(single[1].replace(/''/g, "'"));
        else if (double) {
            try {
                values.push(JSON.parse(double[1]));
            }
            catch {
                return undefined;
            }
...
export function discoverWorkspace(manifests, pnpmText) {
    ...
    if (patterns === undefined)
        return {
            source, status: 'unknown', patterns: [], packagePaths: [],
    ...
    return { source, status: 'discovered', patterns, packagePaths, issues: [] };
}
export function dependencyGraph(manifests, paths, roots) {
    ...
    for (let cursor = 0; cursor < queue.length; cursor += 1) {
        ...
        if (visited.has(name) && (visited.get(name) === false || conditional))
            continue;
        ...
        for (const kind of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
            ...
            if (!isObject(declarations)) {
                issues.push({ kind: 'dependency-declaration', path, reason: `${kind} is not an object` });
                continue;
            }
            for (const [dependency, range] of Object.entries(declarations).sort()) {
                if (kind === 'dependencies' && dependency in optional)
                    continue;
                if (typeof range !== 'string') {
                    issues.push({ kind: 'dependency-declaration', path, reason: `${dependency} has a non-string range` });
                    continue;
                }
                ...
                const edge = { from: name, name: dependency, range, kind, conditional: edgeConditional, resolution };
                const key = `${name}\0${kind}\0${dependency}`;
                edges.set(key, edge);
                if (resolution === 'workspace-candidate')
                    queue.push({ name: dependency, conditional: edgeConditional });
                else if (resolution === 'unresolved')
                    issues.push({ kind: 'dependency-resolution', path, reason: `${dependency}@${range}: inspect alias, protocol, catalog or ambiguous/missing workspace target` });
            }
        }
    }
    return {
