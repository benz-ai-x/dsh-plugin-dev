...
function readJson(path) {
    try {
        ...
        return fail('COMPAT_PROJECT_INPUT', `Expected a JSON object at ${path}`);
    }
}
function projectFile(project, path) {
    if (typeof path !== 'string')
        return fail('COMPAT_PROJECT_INPUT', 'Catalog path must be a string');
    const absolute = resolve(project, path);
    const child = relative(project, absolute);
    if (isAbsolute(child) || child === '..' || child.startsWith(`..${sep}`))
        return fail('COMPAT_PROJECT_INPUT', 'Catalog path must remain inside the project');
    return absolute;
}
export function analyzeProject(options = {}, environment = process.env) {
    ...
    if (existsSync(lockPath)) {
        ...
        if (lock.schemaVersion === 2 && isObject(lock.channels)) {
            ...
            if (!isObject(entry))
                fail('COMPAT_LOCK_CHANNEL', 'Select an existing lock channel with --channel');
            locked = entry;
        }
        else if (lock.schemaVersion === 1 && isObject(lock.upstream) && !options.channel)
            locked = lock;
        else if (!options.base)
            fail('COMPAT_LOCK_SCHEMA', 'Unknown lock schema; inspect its baseline and pass --base plus --root-package explicitly. No code update is required for explicit analysis.');
        else
            issues.push({ kind: 'lock-schema', reason: 'Unknown lock schema bypassed by explicit --base; no audited lock interpretation claimed.' });
    }
    else if (options.channel)
        fail('COMPAT_LOCK_CHANNEL', '--channel requires a project reference lock');
    ...
    if (!base)
        fail('COMPAT_BASE_REQUIRED', 'No baseline commit found. Pass --base REF; never infer the previous tag.');
    ...
    if (!harnessRoot)
        fail('COMPAT_SOURCE_REQUIRED', 'Pass --harness-root PATH or set DSH_HARNESS_ROOT to the candidate Git repository');
    ...
    if (!roots.length && isObject(locked?.catalog)) {
        ...
        if (digest !== locked.catalog.sha256)
            fail('COMPAT_CATALOG_DIGEST', 'Locked catalog digest does not match; use explicit --root-package for an unaudited diagnostic scope');
    ...
    else if (options.capability && !roots.length)
        fail('COMPAT_CAPABILITY_REQUIRED', '--capability requires a locked catalog; otherwise use --root-package');
    ...
    try {
        ...
        if (args.includes('--help') || args.includes('-h'))
            ...
            for (let index = 0; index < args.length; index += 1) {
                ...
                if (flag === '--download-missing') {
                    download = true;
                    continue;
                }
                const value = args[++index];
                if (!['--project', '--harness-root', '--target', '--channel', '--capability', '--base', '--root-package', '--download-dir', '--registry'].includes(flag) || !value || value.startsWith('--'))
            ...
            if ((download && !values['download-dir']) || (!download && (values['download-dir'] || values.registry)))
                fail('COMPAT_USAGE', usage);
